...
app.get("/api/employees", async (_, res) => {
  const r = await pool.query("SELECT employee_id, name, email, language FROM employees");
  res.json(r.rows);
});
...
app.get("/api/pdf/timesheet/:employeeId/:kw/:po", async (req, res) => {
  ...
  let total = 0;
  getWeekDates(kw).forEach(d => {
    const hours = 8.0;
    total += hours;
    doc.text(d.toLocaleDateString("de-DE"), 50, doc.y, { continued: true });
    doc.text(hours.toFixed(2), 200);
  });

  doc.moveDown();
  doc.text(`Gesamtstunden: ${total.toFixed(2)}`);
